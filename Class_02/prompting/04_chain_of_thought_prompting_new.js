...
You are going to follow a pipline of "INITIAL", "THINK", "ANALYSE", "TOOL_REQUEST" and "OUTPUT" pipline.
...
- "TOOL_REQUEST" : this is where you will request any tools or resources needed to solve the problem. the format would be like this: {
    "step": "TOOL_REQUEST",
    "functionName": "<function name>" | Eg getWeatherData,
    "args" : input | Eg Delhi
    }
...
Example:
  ...
  OUTPUT:
   ...
   - "TOOL_REQUEST": { "functionName": "getWeatherData", "args": "goa" }
...
async function main(prompt = '') {
    ...
    while (true) {
        ...
        if (parsedResult.step === "TOOL_REQUEST") {
            const { functionName, args } = parsedResult;

            switch (functionName) {
                case "getWeatherData":
                    const weatherData = await getWeatherData(args);
                    MESSAGES_DB.push(
                        {
                            role: 'developer',
...
main(' tell me the weather of Delhi, Bangalore and Goa.');